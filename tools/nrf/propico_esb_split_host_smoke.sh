...
if [[ ${#LOCAL_WORDS[@]} -lt 30 || ${#REMOTE_WORDS[@]} -lt 30 ]]; then
...
printf 'remote: magic=%s role=%d phase=%d tx_success=%d tx_failed=%d rx_packets=%d ack_payloads=%d anchors_received=%d last_rx_node=%d last_rx_len=%d anchor_seq=%d master_ts_us=%d local_ts_us=%d offset_us=%d raw0=0x%08X raw1=0x%08X seq_gaps=%d offset_min=%d offset_max=%d master_delta=%d local_delta=%d skew=%d skew_min=%d skew_max=%d\n' \
  "${REMOTE_MAGIC}" "${REMOTE_ROLE}" "${REMOTE_PHASE}" "${REMOTE_TX_SUCCESS}" "${REMOTE_TX_FAILED}" "${REMOTE_RX_PACKETS}" "${REMOTE_ACK_PAYLOADS}" "${REMOTE_ANCHORS_RECEIVED}" "${REMOTE_LAST_RX_NODE}" "${REMOTE_LAST_RX_LEN}" "${REMOTE_LAST_ANCHOR_SEQUENCE}" "${REMOTE_LAST_MASTER_TIMESTAMP_US}" "${REMOTE_LAST_LOCAL_TIMESTAMP_US}" "${REMOTE_ESTIMATED_OFFSET_US}" "${REMOTE_ANCHOR_RAW_WORD0}" "${REMOTE_ANCHOR_RAW_WORD1}" "${REMOTE_ANCHOR_SEQUENCE_GAPS}" "${REMOTE_OFFSET_MIN_US}" "${REMOTE_OFFSET_MAX_US}" "${REMOTE_LAST_ANCHOR_MASTER_DELTA_US}" "${REMOTE_LAST_ANCHOR_LOCAL_DELTA_US}" "${REMOTE_LAST_ANCHOR_SKEW_US}" "${REMOTE_ANCHOR_SKEW_MIN_US}" "${REMOTE_ANCHOR_SKEW_MAX_US}"
