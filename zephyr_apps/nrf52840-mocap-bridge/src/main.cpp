...
/* v22 tried midpoint glitch-reject (discard |Δmidpoint| ≥ 10ms jumps
 * as bleed-through). Hypothesis refuted — see docs/RF_V22_FINDINGS.md.
 * Reverted in v23 back to the accept-every-update v21 behaviour.
 *
	...
	if (payload->length >= sizeof(*anchor)) {
		...
		if (!found) {
			...
			if (midpoint_offset_valid) {
				...
				m_bucket = histo_bucket_us(mdiff_abs, midpoint_step_bucket);
			 ...
			 *
			 * v22 tried a glitch-reject here: drop updates with
			 * |mdiff| >= 10ms. It didn't improve cross-Tag span p99
			 * (42.5 -> 47.5 ms) AND it prevented stuck-Tag recovery.
			 * See docs/RF_V22_FINDINGS.md for the hypothesis refutation
			 * and the revised theory (fat-tail is drift-driven, not
			 * jump-driven). Reverted in v23 back to v21 behaviour. */
